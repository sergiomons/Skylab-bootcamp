 ...
 function continueIni() {
    ...
    textName.id="textN";
    textName.name="textNa"  
    textName.type="text";
    textName.style="border: 2px solid ##DEE3C3; border-radius: 5px; height: 26px; width: 300px; margin: 0 10px 0 10px; font-family: arial; font-size: 18px; padding: 10px";
    textName.placeholder="Enter your Name";
    var butName=document.createElement("input");
    shownCom.appendChild(butName);
    butName.type="button";
    butName.className="button";
    butName.value="Confirm";
    butName.onclick= function confName() {
      ...
      butStart.type="button";
      butStart.className="button";
      butStart.name="butStart";
      butStart.value="Start Game";
      shownCom.appendChild(butStart);
      butStart.onclick= function StartG() {
          butPasa.disabled=false;
          butConf.disabled=false;
   ...
   function positionG() {
        ...
        result.innerHTML="Let's go " + userName + "!";
        startTime();
        ask();
   }
 ...
 function ask(){
   if(checkAll()<questions.length) {
      ...
      if(questions[inc].status===0) {
       ...
       document.getElementsByClassName("letter")[inc].style="background-color:#E99848";
      } else {       
        inc++;
       ask();
      }
  ...
  function answerR() {
          ...
          if (answer===questions[inc].answer) {
            ...
            result.style=resultSty;
            result.style.backgroundColor="#3DB513"
  ...
  function exit() {
      ...
      .then((finish) => {
        if (finish) {
           swal({
            ...
            shownRes();
            refresh();
             document.getElementById("showTime").innerHTML = "00";
        } else {   
        }
      });
   }
   ...
   function startAgain() {
           swal({
             title: "Do you want to start Pasapalabra?",
              buttons: true,
               dangerMode: true,
          })
            .then((Start) => {
